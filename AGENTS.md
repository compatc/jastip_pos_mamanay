...
- **send-group proxy: forward URL, jangan fetch blob**: `api/send-group.mjs` sebelumnya fetch image dari URL lalu kirim sebagai FormData blob ke bot. Ini sering gagal (500) karena Vercel function timeout/error saat fetch R2 atau URL lain. Fix: forward `image` URL sebagai JSON ke bot, biar bot yang fetch sendiri. Incident: 10 September 2026, share produk ke grup gambar ga ikut. JANGAN ubah ke fetch blob lagi.
