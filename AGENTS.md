...
- **Rekap order dedup (local + Supabase)**: `buatRekapProduk()` mengambil data dari 2 sumber: local `orders.json` + Supabase `order_items`. Order yang sama muncul di keduanya (bot push ke Supabase). Dedup key: `sender.trim().toLowerCase() + variant.trim().toLowerCase() + qty + timestamp.slice(0,16)` (trunc ke menit). Timestamp local vs Supabase beda detik → trunc ke menit supaya tetap ke-dedup. Tapi order berbeda (menit beda) tetap muncul semua. Incident: 25 Agustus 2026, rekap double untuk Ratih + Wulandari karena dedup lama pakai timestamp penuh (tidak match antara local ISO string vs Supabase `created_at`).
