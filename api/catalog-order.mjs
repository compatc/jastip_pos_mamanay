...
export default async function handler(req, res) {
  ...
  try {
      ...
      if (promoRec !== null) {
        ...
        for (const p of (prods || [])) {
          ...
          if (p.stock >= 15 && (discountedPrice15 - costPrice) >= minProfit) {
            ...
            promoMsg = `≡ƒÅ╖∩╕Å ${p.name} ${bundlePrice.toLocaleString("id-ID")}\n${stockBadge}[stok:${p.stock}]\n≡ƒÄü BUNDLING Beli ${bundleQty} Lebih Hemat!\nHarga normal: Rp${p.sell_price.toLocaleString("id-ID")}/pcs\nBeli ${bundleQty} pcs: *Rp${bundlePrice.toLocaleString("id-ID")}/pcs*\n≡ƒÆí Hemat Rp${bundleSaving.toLocaleString("id-ID")} untuk ${bundleQty} pcs!`;
            ...
            const bundleSaving = normalTotal - bundleTotal;
            promoType = "bundling"; promoValue = bundleDiscount;
            promoMsg = `≡ƒÅ╖∩╕Å ${p.name} ${bundlePrice.toLocaleString("id-ID")}\n${stockBadge}[stok:${p.stock}]\n≡ƒÄü BUNDLING Beli 2 Lebih Hemat!\nHarga normal: Rp${p.sell_price.toLocaleString("id-ID")}/pcs\nBeli 2 pcs: *Rp${bundlePrice.toLocaleString("id-ID")}/pcs*\n≡ƒÆí Hemat Rp${bundleSaving.toLocaleString("id-ID")} untuk 2 pcs!`;
          } else if (sold === 0 && p.stock >= 5 && (discountedPrice20 - costPrice) >= minProfit) {
            promoType = "flash_sale"; promoValue = 20;
            promoMsg = `≡ƒÅ╖∩╕Å ${p.name} ${discountedPrice20.toLocaleString("id-ID")}\n${stockBadge}[stok:${p.stock}]\n≡ƒöÑ FLASH SALE!\nRp${p.sell_price.toLocaleString("id-ID")} ΓåÆ *Rp${discountedPrice20.toLocaleString("id-ID")}* (hemat Rp${(p.sell_price - discountedPrice20).toLocaleString("id-ID")})\nBuruan sebelum kehabisan!`;
          } else if ((discountedPrice15 - costPrice) < minProfit) {
            continue;
          } else {
            promoMsg = `≡ƒÅ╖∩╕Å ${p.name} ${discountedPrice15.toLocaleString("id-ID")}\n${stockBadge}[stok:${p.stock}]\n≡ƒÅ╖∩╕Å DISKON ${promoValue}%\nRp${p.sell_price.toLocaleString("id-ID")} ΓåÆ *Rp${discountedPrice15.toLocaleString("id-ID")}* (hemat Rp${(p.sell_price - discountedPrice15).toLocaleString("id-ID")})\nBuruan sebelum kehabisan!`;
          }
          recs.push({
    ...
    if (url.searchParams.get("action") === "build-catalog") {
      ...
      const { data: prodData } = await sb.from("products").select("id, name, sell_price, stock, stock_type, po_closed, unit, image").order("name");
      ...
      const bR2_ACCOUNT_ID = "3ba62fa119ee4f295a5655776bfdb386";
    ...
    if (isBotOrder) {
      ...
      sendPushNotification(sb, {
        ...
        body: `${customer_name || "Pelanggan"}: ${summary} ΓÇö Rp ${subtotal.toLocaleString("id-ID")}`,
        url: `/orders/${orderId}`,
        type: "new_order",
        orderIds: [orderId],
        amount: subtotal,
    ...
    const itemList = items.map((i) => `ΓÇó ${i.product_name || i.name} x${i.quantity || 1} = Rp ${((i.price || 0) * (i.quantity || 1)).toLocaleString("id-ID")}`).join("\n");
    let waMsg = "≡ƒ¢Æ *Order dari Catalog*\n\n";
    waMsg += "≡ƒæñ " + customer_name.trim() + "\n";
    waMsg += "≡ƒô▒ " + phone.trim() + "\n";
    if (address?.trim()) waMsg += "≡ƒôì " + address.trim() + "\n";
    if (notes?.trim()) waMsg += "≡ƒô¥ " + notes.trim() + "\n";
    waMsg += "\n≡ƒôª *Detail:*\n" + itemList + "\n";
    waMsg += "\n≡ƒÆ░ *Total: Rp " + subtotal.toLocaleString("id-ID") + "*\n";
    waMsg += "≡ƒôï Status: Baru\n\n";
    waMsg += "ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ\n";
    ...
    sendPushNotification(sb, {
      ...
      body: `${customer_name}: ${summary} ΓÇö Rp ${subtotal.toLocaleString("id-ID")}`,
      url: `/orders/${orderId}`,
      type: "new_order",
      orderIds: [orderId],
      amount: subtotal,
