...
// Replace the old buatRekapProduk that only uses local orders
// with one that also queries Supabase
...
const newFunc = `async function buatRekapProduk(nama, judul) {
  // Local orders (from WA bot) - normalize for comparison
...
if (code.includes('async function buatRekapProduk')) {
  console.log('ALREADY HAS async buatRekapProduk with Supabase');
} else if (code.includes('function buatRekapProduk(nama, judul)')) {
  code = code.replace(oldFunc, newFunc);
  console.log('Replaced buatRekapProduk with Supabase version');
} else {
  console.log('Pattern not found');
}

// Also fix callers: await buatRekapProduk(...)
code = code.replace(/await buatRekapProduk\(/g, 'await buatRekapProduk(');
// Make sure callers use await since function is now async
code = code.replace(/const rekap = buatRekapProduk\(/g, 'const rekap = await buatRekapProduk(');
code = code.replace(/await kirim\(sock, chatId, buatRekapProduk\(/g, 'await kirim(sock, chatId, await buatRekapProduk(');
