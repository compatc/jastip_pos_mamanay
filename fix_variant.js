...
// 1. Add parseVariantsFromPromo function
if (!c.includes('parseVariantsFromPromo')) {
  const fn = `function parseVariantsFromPromo(promoText) {
  if (!promoText) return [];
  const variants = [];
...
`;
  c = c.replace('function getOrderKey(o) {', fn + 'function getOrderKey(o) {');
  console.log('1. Added parseVariantsFromPromo');
}
...
if (c.includes(oldLogic)) {
  ...
  console.log('2. Updated order logic with variant matching');
} else {
  console.log('WARNING: old order logic not found');
}

fs.writeFileSync(f, c, 'utf8');
console.log('3. Saved. Restart with: pm2 restart wa-bot');
