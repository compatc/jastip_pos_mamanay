...
function getOrderStatusBadge(order: any): { label: string; cls: string; accent: string } {
  const paid = order.payment_status === "paid" || (order.paid_total >= (order.total - (order.diskon || 0) - (order.kode_unik || 0)) && order.total > 0);
...
export default function CustomerOrders() {
  ...
  function openWaModal() {
    const unpaid = orders.filter((o) => o.paid_total < (o.total - (o.diskon || 0) - (o.kode_unik || 0)) && o.total > 0);
  ...
  const unpaidOrders = orders.filter((o) => o.paid_total < (o.total - (o.diskon || 0) - (o.kode_unik || 0)) && o.total > 0);

  const totalBelanja = orders.reduce((s, o) => s + o.total, 0);
  const totalLunas = orders.filter((o) => o.paid_total >= (o.total - (o.diskon || 0) - (o.kode_unik || 0)) && o.total > 0).length;
  const sisaPiutang = unpaidOrders.reduce((s, o) => s + ((o.total - (o.diskon || 0) - (o.kode_unik || 0)) - (o.paid_total || 0)), 0);
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    if (filter === "dikemas") {
      const isPaid = o.payment_status === "paid" || o.paid_total >= (o.total - (o.diskon || 0) - (o.kode_unik || 0));
  ...
  function renderOrderCard(order: any, isCompletedTab: boolean) {
    ...
    const paid = order.payment_status === "paid" || order.paid_total >= (order.total - (order.diskon || 0) - (order.kode_unik || 0));
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-5 space-y-3 z-10 max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="overflow-y-auto flex-1 min-h-0 space-y-2 -mx-5 px-5">
              {orders.map((order) => {
                const isUnpaid = order.paid_total < (order.total - (order.diskon || 0) - (order.kode_unik || 0)) && order.total > 0;
                const items = itemsByOrder[order.id] || [];
                return (
                  <label
                    key={order.id}
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-5 space-y-3 z-10 max-h-[85vh] flex flex-col overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5 font-semibold">
                        Tagihan: {rupiah((order.total - (order.diskon || 0) - (order.kode_unik || 0)) - (order.paid_total || 0))}
                      </p>
                    </div>
                  </label>
                );
            ...
            {(() => {
              const totalSelectedAmount = unpaidOrders
                ...
                .reduce((s, o) => s + ((o.total - (o.diskon || 0) - (o.kode_unik || 0)) - (o.paid_total || 0)), 0);
              return (
                <div className="flex items-center justify-between pt-2 border-t border-gray-100 shrink-0">
