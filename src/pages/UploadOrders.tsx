...
const TEMPLATE_CSV = `tanggal,pelanggan,produk,harga,jumlah,diskon,ongkir,tipe_bayar,tipe_order,catatan
2025-01-15,Budi,Produk A,50000,2,0,10000,tf,penjualan,
2025-01-15,Budi,Produk B,30000,1,5000,0,qris,penjualan,
2025-01-16,Andi,Produk A,50000,5,0,15000,tf,pembelian,urgent`;
...
export default function UploadOrders() {
  ...
  const { products, loadProducts, loadAllOrders, loadCustomers } = useStore();
    ...
    for (const group of orders) {
      try {
        ...
        const { data: existing } = await supabase
          .from("customers")
          .select("id")
          .ilike("name", group.pelanggan.trim())
          .limit(1)
          .single();
        if (existing) {
