...
export const useStore = create<PosStore>((set, get) => ({
  ...
  loadAllOrders: async () => {
    ...
    try {
      ...
      if (ordersRes.error) {
        console.error("loadAllOrders orders error:", ordersRes.error.message);
        return;
      }
  ...
  loadProducts: async () => {
    ...
    const { data, error } = await supabase
      .from("products")
      .select("id,name,cost_price,sell_price,stock,stock_type,po_closed,supplier,unit,image,images,weight,description,shopee_item_id,shopee_synced_at,created_at")
